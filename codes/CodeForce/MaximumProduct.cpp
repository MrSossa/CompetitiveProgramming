...
void solve(){
    ...
    for1(i,n){
        if (i==cerosIndex[cIndex]){
            if (ans < mul1 || ans < mul2){
                if (mul1 >= mul2){
                    
                    lans = cerosIndex[cIndex-1];
                    rans = n-(r-1);
                }else{
                    
                    lans = l;
                    rans = n-(cerosIndex[cIndex]-1);
                    
                }
            }
            ans = max(ans,max(mul1,mul2));
            cIndex++;
            if (neg[i]-neg[cerosIndex[cIndex]] % 2 == 0){
                r = cerosIndex[cIndex];
                l = i;
            }else{
                int aux = i;
