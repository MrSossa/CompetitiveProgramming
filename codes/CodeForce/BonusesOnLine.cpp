...
void solve(){
	long long n,t,pos,count=0,posl,posr;
    pos = -1;
    cin >> n >> t;
    int nums[n];
    forn(i,n){
    ...
    posr = pos;
    if (nums[pos]==0)posl=pos;
    else posl = pos-1;
    vector<long long> nl(posl+1);
    vector<long long> nl2(posl+1);

    vector<long long> nr(n-posr);
    vector<long long> nr2(n-posr);
    forn(i,pos){
        nl[i] = nums[pos-i];
    }
    for(int i = 0;i<=pos;++i){
